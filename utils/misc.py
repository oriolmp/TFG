...
def create_transformation():
    from Domain_adaptive_WTAL.src.datasets.e2e_lib.videotransforms_gpu import GroupRandomHorizontalFlip, GroupRandomCrop, GroupCenterCrop, \
...
def apply_transformation(imgs, transforms):
    try:
        # Input has shape BxCxTxHxW
        # The transformation takes as input CxTxHxW
        imgs = torch.stack([transforms(img) for img in imgs], 0) # Apply the transformation to the images (data augmentation)

    except Exception as e:
        # traceback.print_exc()
        raise IOError("failed to transform one video")
    return imgs
...
def nested_tensor_from_tensor_list(tensor_list: List[Tensor]):
    ...
    if tensor_list[0].ndim == 3:  # n,c,t
        ...
        max_size = max([video_ft.shape[1] for video_ft in tensor_list])  # [c,t,h,w] or [c,t]
        # min_size = tuple(min(s) for s in zip(*[img.shape for img in tensor_list]))
